...
//            $this->pdf->Cell(200, 45, 'CERTIFICADO DE MATRICULA No. 00' . $row['numero_id'], 0, 0, 'C');
            ...
            $this->pdf->SetXY(20, 20);
            if (!$row['foto'] == NULL) {
                $this->pdf->Image('uploads/imagenes/' . $row['foto'], 30, 95, 25, 30);
            }
            $this->pdf->Cell(200, 130, 'CERTIFICA', 0, 0, 'C');
            $this->pdf->Ln(60);
            $this->pdf->SetFontSize(12);
            $this->pdf->Cell(220, 28, 'Que:', 0, 0, 'C');
            $this->pdf->SetFontSize(12);
            $this->pdf->Ln();
            $this->pdf->Cell(220, -10, $row['nombres'] . ' ' . $row['apellidos'], 0, 0, 'C');
...
//            $this->pdf->Cell(220,15,'Hola sass',0,0,'C');
            ...
            $this->pdf->SetFontSize(12);
            $this->pdf->Ln();
            $this->pdf->Cell(220, 18, ' C.C. No ' . $row['cedula'], 0, 0, 'C');
            $this->pdf->Ln();
            $this->pdf->Cell(220, -3, ' Direccion: ' . $row['domicilio'], 0, 0, 'C');
            $this->pdf->Ln();
            $this->pdf->Cell(220, 11, '  Correo: ' . $row['correo'], 0, 0, 'C');
            $this->pdf->Ln();
            $this->pdf->Cell(220, -3, ' Celular: ' . $row['celular'] . '  Fijo: ' . $row['telefono'], 0, 0, 'C');
...
//$this->SetFontSize(12);
            $this->pdf->Ln();
            $this->pdf->Cell(185, 16, utf8_decode('APROBÓ Y CUMPLIÓ CON LA TOTALIDAD DEL CICLO ACADEMICO, SU CORRESPONDIENTE'), 0, 0, 'C');
            $this->pdf->Ln();
            $this->pdf->Cell(185, -6, utf8_decode('EVALUACIÓN PRESENTANDO LOS REQUISITOS DE CONFORMIDAD A LOS ESTATUTOS, ACUERDOS'), 0, 0, 'C');
            $this->pdf->Ln();
            $this->pdf->Cell(185, 14, utf8_decode('RESOLUCIONES, CÓDIGO DE ÉTICA Y EL ORDENAMIENTO LEGAL, SEGUN RESOLUCION ' . $numat), 0, 0, 'C');
            $this->pdf->Ln();
            $this->pdf->Cell(185, -6, 'NUMERO, 00 DE FECHA,  COMPROBANDOSE SU IDONEIDAD PARA EJERCER', 0, 0, 'C');
            $this->pdf->Ln();
            $this->pdf->Cell(185, 14, 'COMO AVALUADOR PROFESIONAL EN LAS SIGUIENTES ESPECIALIDADES:', 0, 0, 'C');
            $this->pdf->Ln(15);
            ...
            $this->pdf->MultiCell(180, 4, $txt, 0, 'C');
//            echo "<pre>";
//            var_dump($categorias);
//            echo "</pre>";
//            exit();
            $this->pdf->Ln(-35);
            $this->pdf->Cell(180, 80, 'SE LE ASIGNO LA MATRICULA R.N.A P-C. No.' . $numat . ' CON UNA VIGENCIA  HASTA ' . $fechavenava, 0, 0, 'C');
            $this->pdf->Ln();
            $this->pdf->Cell(180, -70, 'Y CON EL REGISTRO DE SUPERINTENDENCIA DE INDUSTRIA Y COMERCIO No ', 0, 0, 'C');
            $this->pdf->Ln();
            $this->pdf->Cell(180, 80, utf8_decode('EL OFICIO ESTARÁ SUJETO AL CUMPLIMIENTO DE LOS ESTATUTOS, REGLAMENTACIONES'), 0, 0, 'C');
            $this->pdf->Ln();
            $this->pdf->Cell(180, -70, utf8_decode('ACTUALIZACIONES PERMANENTES Y DEMÁS REQUISITOS EXIGIDOS POR EL CONSEJO Y POR LA LEY.'), 0, 0, 'C');
            $this->pdf->Ln();
...
//            $this->Image('imgpdf/asopeq.jpg',170,255,23,17);
            $this->pdf->Cell(0, 45, 'PIN:' . $pin, 0, 1, 'C', 0);

            $this->pdf->Image('uploads/codigosqr/' . $pin . '.png', 98, 228, 23, 20);
